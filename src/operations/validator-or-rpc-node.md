...
cluster consensus. You will want to attract a delegation of BTG to your
...
blocks. produced by the
leader. The vote credits are given
...
rent fees for each block that it
produces that is added to the blockchain.

Since all votes in Openos happen on the blockchain, a validator incurs a
transaction cost for each vote that it makes. These transaction fees amount to
approximately 1.0 BTG per day.

> It is important to make sure your validator always has enough BTG in its
...
for blocks that they agree with. The cost of voting can be up to 1.1 BTG per
day.

A voting validator can earn BTG through 2 methods:
...
- Michael Hubbard wrote an
  ...
  that explains the economics of Openos in more depth for stakers and for
  validators.
...
strong understanding of the Openos architecture and the
