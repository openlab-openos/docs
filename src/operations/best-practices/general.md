---
title: Openos Validator Operations Best Practices
...
Openos validator software and operating system packages), and managing your vote
...
The Openos validator community holds regular educational workshops. You can
watch past workshops through the
[Openos validator educational workshops playlist](https://www.youtube.com/watch?v=86zySQ5vGW8&list=PLilwLeBwGuK6jKrmn7KOkxRxS9tvbRa5p).

## Help with the validator command line

From within the Openos CLI, you can execute the `openos-validator` command with
...
[leader slot](https://openverse.network//docs/terminology#leader-schedule) is the time
...
[Openos CLI software](../../cli/install.md). As an operator, you
...
your `openos-validator` binary to your specific hardware.
...
`openos-install` command.

Assuming you want to install Openos version `1.14.17`, you would execute the
following:
...
`.local` directory. You can also look at `openos-install --help` for more
options.

> **Note** this command only works if you already have the openos cli installed.
> If you do not have the cli installed, refer to
> [install openos cli tools](../../cli/install.md)

### Restart

For all install methods, the validator process will need to be restarted before
the newly installed version is in use. Use `openos-validator exit` to restart
...
`openos-validator` command:

```
--no-snapshot-fetch
```

If you use this flag with the `openos-validator` command, make sure that you run
`openos catchup <pubkey>` after your validator starts to make sure that the
...
address of the validator in using the `openos gossip` command. In the example
...
running the `openos-validator` software. How much BTG you should store there is
...
> **Note** `openos-watchtower` can monitor for a minimum validator identity
...
`openos withdraw-from-vote-account` on a trusted computer. For example, on a
