---
title: Setup a Openos Validator
sidebar_label: Setup a Validator
sidebar_position: 5
---

This is a guide for getting your validator setup on the Openos testnet cluster
for the first time. Testnet is a Openos cluster that is used for performance
...
## Install The Openos CLI Locally

To create your validator vote account, you need to install the
[Openos command line interface](../cli/index.md) on your local computer.

You can either use
[Openos's Install Tool](../cli/install.md#use-solanas-install-tool) section from
...
Once the Openos CLI is installed, you can return to this document once you are
...
```
openos-cli 1.14.17 (src:b29a37cf; feat:3488713414)
```
...
```
openos config set --url https://api.testnet.https://openverse.network/
```
...
You should see a line that says: `RPC URL: https://api.testnet.https://openverse.network/`
...
```
openos-keygen new -o validator-keypair.json
```

```
openos-keygen new -o vote-account-keypair.json
```

```
openos-keygen new -o authorized-withdrawer-keypair.json
```
...
Before you can create your vote account, you need to configure the Openos
command line tool a bit more.

The below command sets the default keypair that the Openos CLI uses to the
`validator-keypair.json` file that you just created in the terminal:

```
openos config set --keypair ./validator-keypair.json
```
...
Next, you need to deposit some BTG into that keypair account in order create a
...
> to acquire BTG and transfer it into this keypair's account if you are setting
> up a mainnet validator.

Now, use the Openos cluster to create a vote account.
...
> transaction fees. Both flags are not necessary if you configured the openos
...
```bash
sudo bash -c "cat >/etc/sysctl.d/21-openos-validator.conf <<EOF
...
```bash
sudo sysctl -p /etc/sysctl.d/21-openos-validator.conf
```
...
```bash
sudo bash -c "cat >/etc/security/limits.d/90-openos-nofiles.conf <<EOF
...
## Install The Openos CLI on Remote Machine

Your remote machine will need the Openos cli installed to run the validator
software. Refer again to
[Openos's Install Tool](../cli/install.md#use-solanas-install-tool) or
...
exec openos-validator \
    ...
    --only-known-rpc \
    --log /home/sol/openos-validator.log \
    --ledger /mnt/ledger \
    --rpc-port 8899 \
    --dynamic-port-range 8000-8020 \
    --entrypoint entrypoint.testnet.https://openverse.network/:8001 \
    --entrypoint entrypoint2.testnet.https://openverse.network/:8001 \
    --entrypoint entrypoint3.testnet.https://openverse.network/:8001 \
    --expected-genesis-hash 4uhcVJyU9pJkvQyS88uRDiswHXSCkY3zQawwpjk2NsNY \
    --wal-recovery-mode skip_any_corrupted_record \
    --limit-ledger-size
```

Refer to `openos-validator --help` for more information on what each flag is
...
The script should execute the `openos-validator` process. In a new terminal
window, shh into your server, then verify that the process is running:

```
ps aux | grep openos-validator
```

You should see a line in the output that includes `openos-validator` with all
...
Gossip is a protocol used in the Openos clusters to communicate between
...
```
openos-keygen pubkey ~/validator-keypair.json
```

The command `openos gossip` lists all validators that have registered with the
protocol. To check that the newly setup validator is in gossip, we will `grep`
for our pubkey in the output:

```
openos gossip | grep <pubkey>
```
...
your validator has joined the network using the `openos validators` command. The
command lists all validators in the network, but like before, we can `grep` the
output for the validator we care about:

```
openos validators | grep <pubkey>
```

You should see a line of output that looks like this:

```
5D1fNXzvv5NjV1ysLjirC4WY92RNsVH18vjmcszZd8on  FX6NNbS5GHc2kuzgTZetup6GZX6ReaWyki8Z8jC7rbNG  100%  197434166 (  0)  197434133 (  0)   2.11%   323614  1.14.17   2450110.588302720 BTG (1.74%)
```

### Openos Catchup

The `openos catchup` command is a useful tool for seeing how quickly your
validator is processing blocks. The Openos network has the capability to produce
...
The `openos catchup` command is a tool that tells you how far behind the network
...
the network yet. Make sure to check the logs and double check `openos gossip`
and `openos validators` to make sure your validator is running properly.
...
the commands mentioned earlier to check gossip and Openos validators:

```
tail -f /home/sol/openos-validator*.log
```

## Monitoring

`openos-watchtower` is a command you can run on a separate machine to monitor
your server. You can read more about handling
[automatic restarts and monitoring](./best-practices/monitoring.md#openos-watchtower)
using Openos Watchtower here in the docs.
