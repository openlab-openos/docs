...
### Primary Validator
The following additional `openos-validator` parameters are required to enable
...
following `openos-validator` command-line argument changes:
...
```bash
$ openos-validator wait-for-restart-window --identity validator-keypair.json \
  && openos-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `openos-validator set-identity
