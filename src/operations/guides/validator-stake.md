...
```bash
openos catchup ~/validator-keypair.json
```
...
this step, you should see the “validator-stake-keypair.json” in your Openos
runtime directory.

```bash
openos-keygen new -o ~/validator-stake-keypair.json
```

## Delegate Stake

Now delegate 1 BTG to your validator by first creating your stake account:

```bash
openos create-stake-account ~/validator-stake-keypair.json 1
```

and then delegating that stake to your validator:

```bash
openos delegate-stake ~/validator-stake-keypair.json ~/vote-account-keypair.json
```

> Don’t delegate your remaining BTG, as your validator will use those tokens to
> vote.
...
```bash
openos delegate-stake ~/validator-stake-keypair.json ~/some-other-vote-account-keypair.json
```
...
- View your vote account:`openos vote-account ~/vote-account-keypair.json` This
...
- View your stake account, the delegation preference and details of your
  stake:`openos stake-account ~/validator-stake-keypair.json`
- `openos validators` displays the current active stake of all validators,
  including yours
- `openos stake-history` shows the history of stake warming up and cooling down
  over recent epochs
...
- Once your stake is warmed up, you will see a stake balance listed for your
  validator by running `openos validators`
...
- After your validator is caught up, use the `openos balance` command to monitor
  ...
  fees
- Openos nodes offer a number of useful JSON-RPC methods to return information
...
- `getVoteAccounts` This will tell you how much active stake your validator
  ...
  You can learn more about staking on Openos
...
```bash
openos deactivate-stake ~/validator-stake-keypair.json
```
