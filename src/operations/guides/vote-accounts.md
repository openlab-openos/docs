...
account is needed if you plan to run a validator node on Openos.
...
the validator operator may choose to store only enough BTG on the identity
...
fee. Because transaction fees on Openos are assessed per-signature, having one
...
vote-authorize-voter-checked command.

The vote authority can be changed at most once per epoch. If the authority is
changed with
vote-authorize-voter-checked this will not take effect until the beginning of the next epoch. To support a
smooth transition of the vote signing, `openos-validator` allows the
...
1. Create the new validator identity keypair,
   `openos-keygen new -o ~/new-validator-keypair.json`.
2. Ensure that the new identity account has been funded,
   `openos transfer ~/new-validator-keypair.json 500`.
3. Run
   `openos vote-update-validator ~/vote-account-keypair.json ~/new-validator-keypair.json ~/authorized_withdrawer.json`
...
no longer listed in the `openos leader-schedule` output.

### Vote Account Authorized Voter

The _vote authority_ keypair may only be changed at epoch boundaries and
requires some additional arguments to `openos-validator` for a seamless
migration.

1. Run `openos epoch-info`. If there is not much time remaining time in the
...
2. Create the new vote authority keypair,
   `openos-keygen new -o ~/new-vote-authority.json`.
3. Determine the current _vote authority_ keypair by running
   `openos vote-account ~/vote-account-keypair.json`. It may be validator's
   identity account (the default) or some other keypair. The following steps
   assume that `~/validator-keypair.json` is that keypair.
4. Run
   `openos vote-authorize-voter-checked ~/vote-account-keypair.json ~/validator-keypair.json ~/new-vote-authority.json`.
   The new vote authority is scheduled to become active starting at the next
   epoch.
5. `openos-validator` now needs to be restarted with the old and new vote
...
6. After the cluster reaches the next epoch, remove the
   ...
   `openos-validator`, as the old vote authority keypair is no longer required.

### Vote Account Authorized Withdrawer

No special handling or timing considerations are required. Use the
`openos vote-authorize-withdrawer-checked` command as needed.
...
1. Entity B creates a durable nonce using `openos create-nonce-account`
2. Entity B then runs a `openos vote-authorize-voter-checked` or
   `openos vote-authorize-withdrawer-checked` command, including:
...
3. When the `openos vote-authorize-...-checked` command successfully executes,
   it will output transaction signatures that Entity B must share with Entity A
4. Entity A then runs a similar `openos vote-authorize-voter-checked` or
   `openos vote-authorize-withdrawer-checked` command with the following
   changes:
...
a vote account withdraws all remaining BTG funds to a supplied recipient address
