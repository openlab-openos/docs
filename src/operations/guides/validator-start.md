...
## Configure Openos CLI

The openos cli includes `get` and `set` configuration commands to automatically
set the `--url` argument for cli commands. For example:

```bash
openos config set --url http://api.devnet.openverse.network/
```

While this section demonstrates how to connect to the Devnet cluster, the steps
are similar for the other [Openos Clusters](../../clusters/available.md).
...
```bash
openos transaction-count
```

View the [metrics dashboard](https://scan.openverse.network) for more
detail on cluster activity.

## Enabling CUDA

If your machine has a GPU with CUDA installed \(Linux-only currently\), include
the `--cuda` argument to `openos-validator`.

When your validator is started look for the following log message to indicate
that CUDA is enabled: `"[<timestamp> openos::validator] CUDA is enabled"`
...
Large system clock drift can prevent a node from properly participating in Openos's [gossip protocol](../../validator/gossip.md).  Ensure that your system clock is accurate.  To check the current system clock, use:
...
You can generate a custom vanity keypair using openos-keygen. For instance:

```bash
openos-keygen grind --starts-with e1v1s:1
```
...
```bash
openos-keygen grind --use-mnemonic --starts-with e1v1s:1
```
...
## More Openos CLI Configuration

Now that you have a keypair, set the openos configuration to use your validator
...
```text
Config File: /home/openos/.config/openos/cli/config.yml
RPC URL: http://api.devnet.openverse.network/
WebSocket URL: ws://api.devnet.openverse.network// (computed)
Keypair Path: /home/openos/validator-keypair.json
Commitment: confirmed
```

## Airdrop & Check Validator Balance

Airdrop yourself some BTG to get started:
...
“vote-account-keypair.json” in your Openos runtime directory:
...
```bash
openos create-vote-account ~/vote-account-keypair.json ~/validator-keypair.json ~/authorized-withdrawer-keypair.json
```
...
argument to `openos-validator`. You can specify multiple ones by repeating the argument `--known-validator <PUBKEY1> --known-validator <PUBKEY2>`.
...
openos-validator \
  ...
  --rpc-port 8899 \
  --entrypoint entrypoint.devnet.openverse.network/:8001 \
  --limit-ledger-size \
  --log ~/openos-validator.log
```
...
> `openos-validator --identity ASK ... --authorized-voter ASK ...`
...
example, `openos-validator --dynamic-port-range 11000-11020 ...` will restrict
...
by adding an argument to `--limit-ledger-size` if desired. 
...
Environment="PATH=/bin:/usr/bin:/home/sol/.local/share/openos/install/active_release/bin"
...
`openos-validator` command-line. Ensure that the 'exec' command is used to
start the validator process (i.e. "exec openos-validator ..."). This is
...
The validator log file, as specified by `--log ~/openos-validator.log`, can get
...
launch the process with `exec` (`exec openos-validator ...`) when using logrotate.
...
which starts the openos validator process uses "exec" to do so (example: "exec
openos-validator ..."); otherwise, when logrotate sends its signal to the
...
1. `sudo mkdir /mnt/openos-accounts`
2. Add a 300GB tmpfs partition by adding a new line containing `tmpfs /mnt/openos-accounts tmpfs rw,size=300G,user=sol 0 0` to `/etc/fstab`
...
5. Enable swap with `sudo swapon -a` and mount the tmpfs with `sudo mount /mnt/openos-accounts/`
6. Confirm swap is active with `free -g` and the tmpfs is mounted with `mount`

Now add the `--accounts /mnt/openos-accounts` argument to your `openos-validator`
