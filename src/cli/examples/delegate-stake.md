---
title: Staking BTG with the Openos CLI
pagination_label: "Openos CLI: Staking"
sidebar_label: Staking
---

After you have [received BTG](./transfer-tokens.md), you might consider putting it
to use by delegating _stake_ to a validator. Stake is what we call tokens in a
_stake account_. Openos weights validator votes by the amount of stake delegated
to them, which gives those validators more influence in determining then next
valid block of transactions in the blockchain. Openos then generates new BTG
periodically to reward stakers and validators. You earn more rewards the more
stake you delegate.

:::info
For an overview of staking, read first the
[Staking and Inflation FAQ](https://openverse.network//staking).
:::
...
the
[stake account address](https://openverse.network//docs/economics/staking/stake-accounts#account-address).
No need for a password or encryption here; this keypair will be discarded right
after creating the stake account.

```bash
openos-keygen new --no-passphrase -o stake-account.json
```
...
```bash
openos create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `openos stake-account` command:

```bash
openos stake-account <STAKE_ACCOUNT_ADDRESS>
```

The output will look similar to this:

```text
Total Stake: 5000 BTG
...
[Stake and withdraw authorities](https://openverse.network//docs/economics/staking/stake-accounts#understanding-account-authorities)
can be set when creating an account via the `--stake-authority` and
`--withdraw-authority` options, or afterward with the `openos stake-authorize`
command. For example, to set a new stake authority, run:

```bash
openos stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
openos create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
`openos create-address-with-seed`:

```bash
openos create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`openos create-stake-account`.
...
accounts with the `openos validators` command:
...
`openos delegate-stake`:

```bash
openos delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `openos stake-account` to observe the changes to the
stake account:
...
Delegated Stake: 4999.99771712 BTG
...
Once delegated, you can undelegate stake with the `openos deactivate-stake`
command:

```bash
openos deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `openos withdraw-stake` command:

```bash
openos withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `openos split-stake` command:

```bash
openos split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
