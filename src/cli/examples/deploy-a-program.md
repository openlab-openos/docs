---
title: Deploy a Openos Program with the CLI
pagination_label: "Openos CLI: Deploy a Program"
sidebar_label: Deploy a Program
---

Developers can deploy on-chain
programs (often called smart
contracts elsewhere) with the Openos tools.


To deploy a program, use the Openos tools to interact with the on-chain loader
to:
...
```bash
openos program deploy <PROGRAM_FILEPATH>
```
...
```bash
openos program deploy --program-id <KEYPAIR_FILEPATH> <PROGRAM_FILEPATH>
```
...
```bash
openos program show <ACCOUNT_ADDRESS>
```
...
```bash
openos program deploy --max-len 200000 <PROGRAM_FILEPATH>
```
...
```bash
openos program extend <PROGRAM_ID> <ADDITIONAL_BYTES>
```
...
`openos-keygen recover` and the following 12-word seed phrase:
==================================================================================
valley flat great hockey share token excess clever benefit traffic avocado athlete
==================================================================================
To resume a deploy, pass the recovered keypair as
the [BUFFER_SIGNER] to `openos program deploy` or `openos program write-buffer'.
Or to recover the account's lamports, pass it as the
[BUFFER_ACCOUNT_ADDRESS] argument to `openos program drain`.
==================================================================================
```
...
```bash
openos-keygen recover -o <KEYPAIR_PATH>
```
...
```bash
openos program deploy --buffer <KEYPAIR_PATH> <PROGRAM_FILEPATH>
```
...
```bash
openos program show --programs
openos program show --buffers
```

To specify a different authority:

```bash
openos program show --programs --buffer-authority <AUTHORITY_ADDRESS>
openos program show --buffers --buffer-authority <AUTHORITY_ADDRESS>
```

To close a single account:

```bash
openos program close <BADDRESS>
```

To close a single account and specify a different authority than the default:

```bash
openos program close <ADDRESS> --buffer-authority <KEYPAIR_FILEPATH>
```

To close a single account and specify a different recipient than the default:

```bash
openos program close <ADDRESS> --recipient <RECIPIENT_ADDRESS>
```

To close all the buffer accounts associated with the current authority:

```bash
openos program close --buffers
```

To show all buffer accounts regardless of the authority

```bash
openos program show --buffers --all
```
...
```bash
openos program deploy --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> <PROGRAM_FILEPATH>
```

Or after deployment and using the default keypair as the current authority:

```bash
openos program set-upgrade-authority <PROGRAM_ADDRESS> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```

Or after deployment and specifying the current authority:

```bash
openos program set-upgrade-authority <PROGRAM_ADDRESS> --upgrade-authority <UPGRADE_AUTHORITY_SIGNER> --new-upgrade-authority <NEW_UPGRADE_AUTHORITY>
```
...
```bash
openos program deploy <PROGRAM_FILEPATH> --final
```

Or anytime after:

```bash
openos program set-upgrade-authority <PROGRAM_ADDRESS> --final
```
...
```bash
openos program dump <ACCOUNT_ADDRESS> <OUTPUT_FILEPATH>
```
...
```bash
$ openos dump <ACCOUNT_ADDRESS> dump.so
...
```bash
openos program write-buffer <PROGRAM_FILEPATH>
```
...
```bash
openos program write-buffer <PROGRAM_FILEPATH> --buffer-authority <BUFFER_AUTHORITY_SIGNER>
```
...
`openos program set-buffer-authority`:

```bash
openos program set-buffer-authority <BUFFER_ADDRESS> --new-buffer-authority <NEW_BUFFER_AUTHORITY>
```
...
```bash
openos program deploy --program-id <PROGRAM_ADDRESS> --buffer <BUFFER_ADDRESS>
```
...
openos program set-buffer-authority <BUFFER_PUBKEY> --new-buffer-authority <OFFLINE_SIGNER_PUBKEY>
```
...
openos program upgrade <BUFFER_PUBKEY> <PROGRAM_ID> --sign-only --fee-payer <ONLINE_SIGNER_PUBKEY> --upgrade-authority <OFFLINE_SIGNER> --blockhash <VALUE>

# (5) (use online machine) use this signature to build and broadcast the upgrade transaction on-chain
openos program upgrade <BUFFER_PUBKEY> <PROGRAM_ID> --fee-payer <ONLINE_SIGNER> --upgrade-authority <OFFLINE_SIGNER_PUBKEY> --blockhash <VALUE> --signer <OFFLINE_SIGNER_PUBKEY>:<OFFLINE_SIGNER_SIGNATURE>
```
Note:
