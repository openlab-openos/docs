...
As the Openos developer ecosystem grows, so does the need for clear expectations around
breaking API and behavior changes affecting applications and tooling built for Openos.
In a perfect world, Openos development could continue at a very fast pace without ever
...
- Openos software releases include APIs, SDKs, and CLI tooling (with a few [exceptions](#exceptions)).
- Openos software releases follow semantic versioning, more details below.
...
The Openos RPC API, Rust SDK, CLI tooling, and SBF Program SDK are all updated and shipped
along with each Openos software release and should always be compatible between `PATCH`
...
- `beta` software that runs on the Openos Testnet cluster
- `stable` software that run on the Openos Mainnet Beta and Devnet clusters
...
releases (e.g. 1.4.0) and are first run on Openos's Testnet cluster. While running
...
New Openos runtime features are feature-switched and manually activated. Runtime features
...
Openos provides publicly available RPC API nodes for all developers to use. The Openos team
...
discourage dependence upon Openos operated nodes.
...
The Web3.JS SDK also follows semantic versioning specifications but is shipped separately from Openos
software releases.
